...
# rooms = [
#     {'id':1 , 'name':'lets leanrn java'},
#     {'id':2 , 'name':'lets leanrn c#'},
#     {'id':3 , 'name':'lets leanrn python'},
# ]
...
def registerPage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request,"error occured during registritation")
...
def deleteMessage(request,pk):
    ...
    if request.method == 'POST':
        message.delete()
        print(request.META.get('HTTP_REFERER','/s'))
        return redirect('home')
    context = {'obj':message}
    return render(request,'base/delete.html',context)
